...
struct test_roll : public test_case {
    ...
    std::string vars() override {
        return VARS_TO_STR5(type, shift0, shift1, shift3, shift4);
    }

    test_roll(ggml_type type = GGML_TYPE_F32, int shift0 = 3, int shift1 = -2, int shift3 = 1, int shift4 = -1)
        : type(type), shift0(shift0), shift1(shift1), shift3(shift3), shift4(shift4) {}
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    for (int64_t d_conv : {3, 4}) {
        for (int64_t d_inner: {1024, 1536, 2048}) {
            ...
            test_cases.emplace_back(new test_ssm_conv(GGML_TYPE_F32, {32, d_inner, 1, 1}, {d_conv, d_inner, 1, 1}));
            test_cases.emplace_back(new test_ssm_conv(GGML_TYPE_F32, {128, d_inner, 1, 1}, {d_conv, d_inner, 1, 1}));
            test_cases.emplace_back(new test_ssm_conv(GGML_TYPE_F32, {4, d_inner, 4, 1}, {d_conv, d_inner, 1, 1}));
            test_cases.emplace_back(new test_ssm_conv(GGML_TYPE_F32, {8, d_inner, 4, 1}, {d_conv, d_inner, 1, 1}));
            test_cases.emplace_back(new test_ssm_conv(GGML_TYPE_F32, {32, d_inner, 4, 1}, {d_conv, d_inner, 1, 1}));
            test_cases.emplace_back(new test_ssm_conv(GGML_TYPE_F32, {128, d_inner, 4, 1}, {d_conv, d_inner, 1, 1}));
        }
    }
                    ...
                    for (float af : { 1.0f, 1.4245f }) {
                        for (ggml_type type : {GGML_TYPE_F32, GGML_TYPE_F16}) {
                            for (bool ff : {false, true}) { // freq_factors
                                for (float v : { 0, 1 }) {
                                    test_cases.emplace_back(new test_rope(type, {128,  32, 2, b}, 128, 0, 512, fs, ef, af, ff, v, fw)); // llama 7B

                                    if (all) {
                                        test_cases.emplace_back(new test_rope(type, {128,  40, 2, b}, 128, 0, 512, fs, ef, af, ff, v, fw)); // llama 13B
                                        test_cases.emplace_back(new test_rope(type, {128,  52, 2, b}, 128, 0, 512, fs, ef, af, ff, v, fw)); // llama 30B
                                        test_cases.emplace_back(new test_rope(type, {128,  64, 2, b}, 128, 0, 512, fs, ef, af, ff, v, fw)); // llama 65B
                                    }

                                    if (all) {
                                        test_cases.emplace_back(new test_rope(type, { 64,   1, 2, b},  64, 2, 512, fs, ef, af, ff, v, fw)); // neox (falcon 7B)
                                        test_cases.emplace_back(new test_rope(type, { 64,  71, 2, b},  64, 2, 512, fs, ef, af, ff, v, fw)); // neox (falcon 7B)
                                        test_cases.emplace_back(new test_rope(type, { 64,   8, 2, b},  64, 2, 512, fs, ef, af, ff, v, fw)); // neox (falcon 40B)

                                        test_cases.emplace_back(new test_rope(type, { 80,  32, 2, b},  20, 0, 512, fs, ef, af, ff, v, fw));
                                        test_cases.emplace_back(new test_rope(type, { 80,  32, 2, b},  32, 0, 512, fs, ef, af, ff, v, fw));
                                        test_cases.emplace_back(new test_rope(type, { 80,  32, 4, b},  32, 0, 512, fs, ef, af, ff, v, fw));

                                        test_cases.emplace_back(new test_rope(type, { 80,  32, 2, b},  20, 2, 512, fs, ef, af, ff, v, fw)); // neox (stablelm)
                                        test_cases.emplace_back(new test_rope(type, { 80,  32, 2, b},  32, 2, 512, fs, ef, af, ff, v, fw)); // neox (phi-2)
                                        test_cases.emplace_back(new test_rope(type, { 80,  32, 4, b},  32, 2, 512, fs, ef, af, ff, v, fw)); // neox (phi-2)
                                    }

                                    if (all) {
                                        test_cases.emplace_back(new test_rope(type, {128,  12, 2, b}, 128, GGML_ROPE_TYPE_MROPE,  512, fs, ef, af, ff, v, fw)); // rope_multi,m-rope (qwen2vl 2B)
                                        test_cases.emplace_back(new test_rope(type, {128,  28, 2, b}, 128, GGML_ROPE_TYPE_MROPE,  512, fs, ef, af, ff, v, fw)); // rope_multi,m-rope (qwen2vl 7B)
                                        test_cases.emplace_back(new test_rope(type, {128,  12, 2, b},  20, GGML_ROPE_TYPE_MROPE,  512, fs, ef, af, ff, v, fw));
                                        test_cases.emplace_back(new test_rope(type, {128,  28, 2, b},  32, GGML_ROPE_TYPE_MROPE,  512, fs, ef, af, ff, v, fw));
                                        test_cases.emplace_back(new test_rope(type, { 80,  16, 2, b},  80, GGML_ROPE_TYPE_VISION, 512, fs, ef, af, ff, v, fw)); // rope_multi,m-rope (qwen2vl ViT)
                                    }
                                    
                                    test_cases.emplace_back(new test_rope(type, { 64, 128, 2, b},  64, 2, 512, fs, ef, af, ff, v, fw)); // neox (falcon 40B)
                                }
                            }
    ...
    test_cases.emplace_back(new test_roll(GGML_TYPE_F16));
    test_cases.emplace_back(new test_roll(GGML_TYPE_BF16));
